...
public class QuizQuestionService(IUnitOfWork unitOfWork) : IQuizQuestionService
{
    public async ValueTask<QuizQuestion> CreateAsync(QuizQuestion quizQuestion)
    {
        ...
        if (existQuizQuestion is not null)
            throw new AlreadyExistException($"QuizQuestion is already exists" +
            $"QuizId={quizQuestion.QuizId}, QuestionId={quizQuestion.QuestionId}");
    ...
    {
        var existQuizQuestion = await unitOfWork.QuizQuestions
            .SelectAsync(quizQuestion => quizQuestion.Id == id && !quizQuestion.IsDeleted, includes: ["Quiz", "Question"])
            ?? throw new NotFoundException($"QuizQuestion is not found with this ID={id}");

        return existQuizQuestion;
    }

    public async ValueTask<IEnumerable<QuizQuestion>> GetAllAsync(PaginationParams @params, Filter filter, string search = null)
    {
        var quizQuestions = unitOfWork.QuizQuestions
            .SelectAsQueryable(expression: quizQuestions => !quizQuestions.IsDeleted, includes: ["Quiz", "Question"], isTracked: false)
            .OrderBy(filter);
